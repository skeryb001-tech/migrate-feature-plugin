...
当需求包含“完整功能、原样迁移、所有功能、一比一”等表述时，会自动进入严格迁移模式：先盘点源页面的组件、模板、样式、状态、API、鉴权、支付、额度、路由、埋点、资源和生命周期，再生成可解析的功能矩阵和“源渲染入口 → 目标渲染入口”JSON 渲染契约。存在未实现功能、未经证明的相似组件替代、目标路由未激活或 UI 缺少真实运行时/视觉证据时，验收会明确阻断，不会把静态页面近似实现当作完成。
